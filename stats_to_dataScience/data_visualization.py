...
- 학과명 명칭이 변경된 시점에서 두 그룹으로 나눈뒤 키워드 분석 : 2020년 10월 변경
   변경 전 : 2017년 1월 ~ 2020년 9월 30일 / 변경 후 : 2020 10월 01일 ~ 2024년 12월 31일
...
def save_fig(filename):
    plt.savefig(f'./stats_to_dataScience/data/img/{filename}.png', dpi=300, bbox_inches='tight')
...
# [데이터, 통계] 총 건수 : 77508
keywords = ['데이터','통계']
news_df = pd.read_pickle(f"./stats_to_dataScience/data/news_tokenized_{keywords}.pkl")
print(news_df.info())
...
plt.title('연도별 "데이터/통계" 관련 뉴스 기사 수 추이 (2017-2024)', fontsize=15, pad=20)
...
plt.savefig("./stats_to_dataScience/data/img/yearly_news_trend.png", dpi=300)
plt.tight_layout()
#plt.show()
...
# 변경 전/후 워드 클라우드
bf_top50, bf_top10 = generate_compare_cloud(group_before, "변경전 (2017-2020)", 'wc_before_stat', 'YlGnBu')
af_top50, af_top10 = generate_compare_cloud(group_after, "변경후 (2021-2024)", 'wc_after_ds', 'Accent')
...
def plot_top_keywords_bar(common_words, title, filename, color):
    ...
    # 이미지 저장 및 출력
    plt.savefig(f'./stats_to_dataScience/data/img/{filename}.png')
    plt.show()

# 변경 전 상위 키워드 vs 변경 후 상위 키워드 bar 그래프
plot_top_keywords_bar(bf_top10, "변경 전 상위 키워드 Top 50", "bar_rank_before", "Blues_d")
plot_top_keywords_bar(af_top10, "변경 후 상위 키워드 Top 50", "bar_rank_after", "Oranges_d")

print("---------------------학과명 전후 시점에서 키워드 연관도 분석 : networkx------------------------")
...
# 실행: 변경 전과 후 비교, 전체기간
build_keyword_network(group_before, "변경 전: 키워드 연관 네트워크", "network_before", top_n=25)
build_keyword_network(group_after, "변경 후: 키워드 연관 네트워크", "network_after", top_n=25)
build_keyword_network(news_df, "전체 기간: 키워드 연관 네트워크", "network_total", top_n=25)

print("---------------------전체 vs 시점별 관계 비교 코드 : networkx------------------------")
